  ...
  document.querySelectorAll(".workspace").forEach((el) => {
    el.addEventListener("click", async (e) => {
      ...
      if (!el.classList.contains("reserved") && bookingModal) {
        bookingModalEl.querySelector(
          ".modal-title"
        ).innerText = `Reserve ${workspaceTitle}?`;
        // Set data-workspace-id on the form
        const bookingForm = bookingModalEl.querySelector(
          'form[data-type="new-booking"]'
        );
        if (bookingForm) {
          ...
          const startTime = checkForm.querySelector(
            '[name="start_time"]'
          ).value;
            ...
            if (data.success) {
              ...
              workspaceDetails = `
                      <div class='workspace'>
                        ...
                        <p>Amenities: <span>${data.workspace.amenities.join(
                          ", "
                        )}</span></p>
                        <p>Description: <span>${
        ...
        cancelModalEl.querySelector(
          ".modal-title"
        ).innerText = `Cancel booking?`;
        const bookingId = el.getAttribute("data-booking-id");

        const cancelForm = cancelModalEl.querySelector(
        ...
        if (cancelForm && bookingId) {
          ...
          try {
            ...
            if (data.success) {
              ...
              cancelModalEl.querySelector(".booking-details").textContent =
                "Booking not found.";
            }
          } catch (err) {
            cancelModalEl.querySelector(".booking-details").textContent =
